      ...
      <div className="game-card rounded-xl p-0 overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 p-6 md:p-8 bg-[#181524]">
          {round.images.map((image) => {
            ...
            if (hasSelected) {
              if (isSelected) {
                containerClass = "border-[#00FF9D] scale-100 ring-4 ring-[#00FF9D]/30 z-10 cursor-pointer";
                shadowClass = "shadow-[0px_0px_30px_rgba(0,255,157,0.4)]";
              } else {
                // Keep clickable but visually de-emphasize
                containerClass = "border-gray-800 opacity-60 scale-95 cursor-pointer hover:opacity-100 hover:border-white/20";
                shadowClass = "hover:shadow-[4px_4px_0px_#00FF9D]";
              }
            }
            ...
            return (
              <div 
                key={image.id}
                // Allowed to fire every time to update the selection
                onClick={() => onSelect(round.id, image.id)}
              ...
              >
                ...
                <div className={`relative bg-black border-4 ${containerClass} transition-all duration-300 aspect-square overflow-hidden`}>
                   ...
                   {/* Selection Overlay */}
                   {isSelected && (
                      <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/40 backdrop-blur-[2px] animate-in zoom-in duration-200">
                         <div className="transform rotate-[-6deg] px-6 py-3 border-4 border-black shadow-[6px_6px_0px_rgba(0,0,0,0.5)] flex items-center gap-3 font-heading text-2xl uppercase bg-[#00FF9D] text-black">
                            <Check size={28} />
                            SELECTED
                         </div>
                      </div>
